...
#load("~/git/rcpp-logs/results/result-20151113-090023.RData")
...
goodPkgs <- c("accelerometry", "AdaptiveSparsity", "AHR", "algstat", "ALKr", 
              ...
              "gmum.r", "gmwm", "gMWT", "GPareto", "GPvam", "gRain", "graphicalVAR", 
              ...
              "protViz", "pryr", "psd", "psgp", "pvar", "QRM", "quadrupen", 
...
oldoldbadPkg <- c("dfcomb", "miceadds", "optmatch", "rgeolocate", "RStoolbox", 
                  "satellite")

oldbadPkg <- c("cycleRtools", "dplyr", "forestFloor", "GPareto", "LANDD", 
               "lme4", "mice", "miceadds", "pedometrics", "pROC", 
               "RStoolbox", "satellite", "tbart", "wsrf")

badPkg <- c("cycleRtools", "dplyr", "forestFloor", "LANDD", "lme4", "mice", 
...
baddepends <- c(
    ...
    "forestFloor",      # builds, bombs in tests: "there is no package called 'trimTrees'"
    "LANDD",            # Packages required but not available: ‘GOstats’ ‘GOSemSim’
    "lme4",             # convergence issue, internal to lme4?
    "mice",             # builds, bombs in examples: "Package 'pan' needed fo this function"
    "miceadds",         # builds, bombs in examples: "there is no package called ‘jomo’" -- Rmose"
    ...
    "steadyICA"         # builds, bombs in examples: "there is no package called ‘ProDenICA’"
    )
...
bad4unclear <- c(
    "dplyr",            # needed#define to not redfine complex operator<<()
    "purrr"             # needed#define to not redfine complex operator<<()
)
...
allPkg <- c("accelerometry", "AdaptiveSparsity", "AHR", "algstat", "ALKr", 
            ...
            "StereoMorph", "stm", "StMoSim", "stochvol", "stocks", "stosim", 
            "strataG", "stream", "strum", "survAccuracyMeasures", "surveillance", 
            "survSNP", "SVMMatch", "synchronicity", "synlik", "systemicrisk", 
            "tagcloud", "TAM", "TAQMNGR", "TauStar", "tbart", "tcR", "TDA", 
            "TED", "termstrc", "TESS", "textreg", "textreuse", "TFMPvalue", 
            "tidyr", "timma", "TMB", "tmg", "tmlenet", "tnam", "treeclim", 
            "treescape", "trustOptim", "tvd", "unmarked", "urltools", "V8", 
            "VarSelLCM", "VideoComparison", "VIM", "waffect", "walkr", "webreadr", 
            "wingui", "wordcloud", "wsrf", "XBRL", "xml2", "yakmoR", "zic")
